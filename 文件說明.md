...
陳士弘：程式透過網頁爬蟲技術，提高效率並節省時間。過程中需要掌握 HTML 結構、CSS 選擇器、HTTP 請求等基礎知識，並運用工具如 Python 的 requests 和 BeautifulSoup。實踐中遇到的挑戰包括反爬機制、動態加載頁面及資料清洗，但這些困難也促進了我的技能提升。

林柏佑：因我們這次的報告題目較大膽所以我很擔心會有甚麼後續問題不過我們已盡量用沒那麼露骨的方式去呈現這次的報告，除此之外這次的報告還是比較有趣的因為N網在瀏覽和保存資料這塊不太方便，而我們的題目一定程度上解決這問題，雖我這次沒參與程式的研發但我還是像兩位組員學到了不少希望下次還有類似的機會。

廖緯鴻：這次的期末作業的題目十分有趣，在實作的過程中雖然自己已經盡量以考慮程式擴充性的原則下去撰寫，但是在要添加新功能或是作修改的時候，才發現當時的自己思慮依舊有考慮不周全之處，導致整個函式的邏輯可能需要做重新設計，讓我再次感嘆自己的程式能力有待加強。由於我沒有太多有意思的想法，因此在功能開發的選擇上我希望能夠以簡單好用為主，所以在功能上只設計介面、批量下載以及歷史紀錄的功能。在這次的分組上我很感謝陳士弘同學完成了網頁爬蟲的雛形，讓我在開發上節約了不少的時間，可以把更多的時間關注在額外功能的開發上。

## 附註：

### 工作分配

程式撰寫：陳士弘(爬蟲邏輯)、廖緯鴻(下載指定範圍頁數、批量下載、歷史紀錄、介面)
文件撰寫：廖緯鴻
文件列印：林柏佑
簡報製作：林柏佑
